...
const UpdateListing = () => {
    ...
    async function handleSubmit(e){
        ...
        try {
            ...
            const res=await fetch(`/api/listing/update/${params.listingId}`,{method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(uploadFormData)});
            const data=await res.json();
            setLoading(false);
            if(data.success===false){
    ...
    return (
      <main className='p-3 mx-auto max-w-4xl'>
          <h1 className='text-3xl text-center my-7 font-semibold'>Update Listing</h1>
